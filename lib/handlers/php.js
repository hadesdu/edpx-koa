...
module.exports = exports = function(opt_handler, opt_suffix, opt_rewrite) {
    ...
    return function *(next) {
        ...
        // 返回码不为0，返回500
        // 在开发机上测试时发现返回码255是未知错误，php(5.2.17)正确执行但是返回码是255
        // 因此，在开发机上运行时在php中使用die(xxx)这里是没有办法拿到xxx的，永远都是255
        // mac + php(5.5.11)正确返回了状态码0
        if (response.code && response.code !== 255) {
            this.status = 500;
